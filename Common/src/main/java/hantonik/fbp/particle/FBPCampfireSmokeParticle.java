...
public class FBPCampfireSmokeParticle extends CampfireSmokeParticle implements IKillableParticle {
    ...
    protected FBPCampfireSmokeParticle(ClientLevel level, double x, double y, double z, double xd, double yd, double zd, boolean isSignal, TextureAtlasSprite sprite) {
        ...
        this.multiplier = FancyBlockParticles.CONFIG.campfireSmoke.isRandomFadingSpeed() ? Mth.clamp((float) FBPConstants.RANDOM.nextDouble(0.85D, 1.1D), 0.95F, 1.0F) : 0.95F;
    ...
    public void render(VertexConsumer buffer, Camera info, float partialTick) {
        ...
        this.putCube(buffer, cube, new Vec2[] { new Vec2(u1, v1), new Vec2(u1, v0), new Vec2(u0, v0), new Vec2(u0, v1) }, light, this.rCol, this.gCol, this.bCol, alpha, FancyBlockParticles.CONFIG.global.isCartoonMode());
    }

    private void putCube(VertexConsumer buffer, Vec3[] cube, Vec2[] uv, int light, float rCol, float gCol, float bCol, float alpha, boolean cartoon) {
        var brightness = 1.0F;
