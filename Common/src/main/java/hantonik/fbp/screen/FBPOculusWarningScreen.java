...
public class FBPOculusWarningScreen extends Screen {
    ...
    protected void init() {
        ...
        this.message = MultiLineLabel.create(this.font, Component.translatable("gui.fbp.shaders_warning.message", OCULUS_COMPONENT), this.width - 45);
        this.report = MultiLineLabel.create(this.font, Component.translatable("gui.fbp.shaders_warning.report", OCULUS_COMPONENT), this.width - 45);

        this.continueButton = this.addRenderableWidget(new Button(this.width / 2 - 150 - 5, this.height - 84 / 2 + 4 / 2, 150, 20, Component.translatable("button.fbp.common.continue"), button -> this.onClose()));
        this.continueButton.active = false;

        this.dontShowAgainButton = this.addRenderableWidget(new Button(this.width / 2 + 5, this.height - 84 / 2 + 4 / 2, 150, 20, Component.translatable("button.fbp.shaders_warning.dont_show_again"), button -> {
