...
public class FBPTerrainScreen extends FBPAbstractOptionsScreen {
    ...
    protected void initOptions() {
        ...
        }, () -> (!this.config.terrain.isInfiniteDuration() && !this.config.global.isInfiniteDuration()) && !FancyBlockParticles.CONFIG.global.isLocked(), new TranslationTextComponent("tooltip.fbp.common.max_lifetime").append("\n").append("\n").append(new TranslationTextComponent("tooltip.fbp.default")).append(new StringTextComponent(String.valueOf(FBPConfig.DEFAULT_CONFIG.terrain.getMaxLifetime())).withStyle(TextFormatting.YELLOW))));

        Button particleBlacklistButton = this.openScreenButton(new TranslationTextComponent("screen.fbp.terrain.particle_blacklist").append("..."), () -> null /*new FBPParticleBlacklistScreen(this, this.config)*/, new TranslationTextComponent("tooltip.fbp.common.option_unsupported")); // TODO
